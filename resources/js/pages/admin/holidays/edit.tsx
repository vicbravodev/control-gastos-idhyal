...
export default function AdminHolidaysEdit({ holiday }: { holiday: Holiday }) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: dashboard() },
        { title: 'Días festivos', href: HolidayController.index.url() },
        {
            title: 'Editar',
            href: HolidayController.edit.url(holiday.id),
        },
    ];
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Editar día festivo" />
            <div className="mx-auto flex w-full max-w-2xl animate-fade-in flex-col gap-5 p-4 sm:p-6">
                <Heading
                    title="Editar día festivo"
                    description="Actualiza los datos del festivo. El cambio afectará a futuros cálculos de vacaciones."
                />

                <form
                    onSubmit={(e) => {
                        e.preventDefault();
                        form.put(HolidayController.update.url(holiday.id));
                    }}
                >
                    <Card>
